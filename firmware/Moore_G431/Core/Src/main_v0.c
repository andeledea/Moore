...
void setAxis(uint8_t com)
{
	switch(com)
	{
		...
		if (axis->setV == true)
		{
			if (buf > 0) axis->T = map(buf, 255, 1, axis->MINT, axis->MAXT);
			else
			{
				axis->setV = false;
				axis->T = axis->MAXT;
		...
		else
		{
			...
			{
				case 'L': //lock
					HAL_GPIO_WritePin(axis->axisEnableGPIO, axis->axisEnablePin, GPIO_PIN_SET);
					HAL_UART_Transmit(&hlpuart1, "[Disabling motor]", sizeof("[Disabling motor]"), 100);
					break;

				case 'U': //unlock
					HAL_GPIO_WritePin(axis->axisEnableGPIO, axis->axisEnablePin, GPIO_PIN_RESET);
					HAL_UART_Transmit(&hlpuart1, "[Enabling motor]", sizeof("[Enabling motor]"), 100);
					break;

				case 'F': //direction forward
					HAL_GPIO_WritePin(axis->axisDirectionGPIO, axis->axisDirectionPin, GPIO_PIN_RESET);
					HAL_UART_Transmit(&hlpuart1, "[Setting direction fore]", sizeof("[Setting direction fore]"), 100);
					break;

				case 'B': //direction back
					HAL_GPIO_WritePin(axis->axisDirectionGPIO, axis->axisDirectionPin, GPIO_PIN_SET);
					HAL_UART_Transmit(&hlpuart1, "[Setting direction back]", sizeof("[Setting direction back]"), 100);
					break;

				case 'V': //set speed
					axis->setV = true;
					HAL_UART_Transmit(&hlpuart1, "[Setting velocity]", sizeof("[Setting velocity]"), 100);
					break;
			}
		}
	}
...
uint16_t map(uint16_t x, uint16_t in_min, uint16_t in_max, uint16_t out_min, uint16_t out_max) {
  return out_max/((in_max - x) * (out_max/out_min - 1) / (in_max - in_min) + 1);
}
/* USER CODE END 0 */
...
int main(void)
{
  ...
  HAL_UART_Receive_IT(&hlpuart1, &buf, 1); //start sensing
  /* USER CODE END 2 */
