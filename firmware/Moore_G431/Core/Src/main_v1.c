...
{
	/**
	 * Change this function to receive two byte buffers and concatenati them in one uint16_t
	 * so that we can have more precise control of the speed of the motor
	 */
...
void setAxis(uint8_t com)
{
	uint8_t fil;
	fil = com & 0b11000000;
	switch(fil)  // axis choice from first two bits of fil
	{
		case 0b00000000:
			axis = &xaxis;
	    	break;
	...
	if (axis->setV == true)
	{
		if (fil > 0) axis->T = map(fil, 63, 1, axis->MINT, axis->MAXT);
		else  // 0 was sent stop active axis
		{
			axis->setV = false;
			axis->T = axis->MAXT;
	...
	{
		if (fil & 0b00000001) // accept command only if last bit is 1
		{
			// fil has 6 bits: x x x [v] [f/b] [l/u]

			//lock [enable pin set], unlock [enable pin reset]
			HAL_GPIO_WritePin(axis->axisEnableGPIO, axis->axisEnablePin, fil & 0b00000010);

			//direction forward [direction reset], direction back [direction set]
			HAL_GPIO_WritePin(axis->axisDirectionGPIO, axis->axisDirectionPin, fil & 0b00000100);

			//set speed
			if ((fil & 0b00000010) == 0) // if unlocked
			{
				axis->setV = true;
			}
		}
